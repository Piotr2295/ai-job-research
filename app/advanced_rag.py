...
class QueryExpansionRetriever:
    ...
    def _expand_query(self, query: str) -> List[str]:
        ...
        Given the following query, generate 3 related search queries that would help find more comprehensive information.
...
class HybridRetriever:
    ...
    def __init__(self, vectorstore: VectorStore, documents: List[Document], k: int = 5):
        self.vectorstore = vectorstore
        ...
        self.semantic_retriever = self.vectorstore.as_retriever(search_kwargs={"k": self.k})
...
def create_advanced_rag_chain(vectorstore: VectorStore, documents: List[Document]):
    ...
    You are an expert career advisor helping with job search and skill development.
    Use the following context to provide accurate, helpful information.
    ...
    Provide a comprehensive answer based on the context. Include specific examples and actionable advice.
    """)
...
def create_conversational_rag_chain(vectorstore: VectorStore):
    ...
    # Create a simple conversational chain without deprecated ConversationalRetrievalChain
    def format_docs(docs):
        return "\n\n".join(doc.page_content for doc in docs)
...
class RAGEvaluator:
    ...
    def evaluate_retrieval(self, query: str, ground_truth_docs: List[str], k: int = 5) -> Dict[str, float]:
        ...
        # Calculate metrics
        precision = len(set(retrieved_content) & set(ground_truth_docs)) / len(retrieved_content) if retrieved_content else 0
        recall = len(set(retrieved_content) & set(ground_truth_docs)) / len(ground_truth_docs) if ground_truth_docs else 0
    ...
    def evaluate_generation(self, question: str, generated_answer: str, context_docs: List[Document]) -> Dict[str, Any]:
        """Evaluate generation quality using LLM-as-judge"""

        evaluation_prompt = PromptTemplate.from_template("""
        Evaluate the quality of this AI-generated answer based on the question and context.
...
def test_advanced_rag_pipeline():
    ...
    test_docs = [
        Document(
            page_content="LangChain is a framework for building applications with LLMs. It provides components for chains, agents, and memory management.",
            metadata={"title": "LangChain Overview", "type": "framework"}
        ),
        Document(
            page_content="Retrieval-Augmented Generation (RAG) combines retrieval systems with generative models to provide accurate, up-to-date responses.",
            metadata={"title": "RAG Explanation", "type": "technique"}
        ),
        Document(
            page_content="Vector databases like Pinecone and FAISS enable efficient similarity search for embeddings in production applications.",
